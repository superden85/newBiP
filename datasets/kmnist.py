...
class KMNIST:
    ...
    def __init__(self, args, normalize=False):
        ...
        self.norm_layer = transforms.Normalize(mean=[0.5], std=[0.5])  # Adjust mean and std for KMNIST
    ...
    def data_loaders(self, **kwargs):
        trainset = datasets.KMNIST(  # Use KMNIST dataset
            root=os.path.join(self.args.data_dir, "KMNIST"),  # Change the dataset folder
            train=True,
            download=True,
            transform=self.tr_train,
        )

        valset = datasets.KMNIST(
            root=os.path.join(self.args.data_dir, "KMNIST"),  # Change the dataset folder
            train=True,
            download=True,
            transform=self.tr_train,
        ...
        testset = datasets.KMNIST(  # Use KMNIST dataset
            root=os.path.join(self.args.data_dir, "KMNIST"),  # Change the dataset folder
            train=False,
            download=True,
